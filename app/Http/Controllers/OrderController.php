...
class OrderController extends Controller
{
    ...
    private function formatOrder($orderData, $orderDetail)
    {
        return [
                ...
                'discountPercent' => $orderData->cus_discount_percent,
    ...
    public function create()
    {
        ...
        $products = DB::select("
            SELECT
                ...
                p.name,
                p.image,
                q.quantity as product_quantity,
...
//        dd($products);
    ...
    public function show(Order $order)
    {
        $orderData = DB::table('orders as od')
            ...
            ->select('od.*', 'c.name as cus_name', 'c.phone', 'c.email', 'c.debt as cus_debt',
                ...
                'adr.address', 'w.name as ward', 'a.name as area', 'cc.name as cus_cat_name', 'cc.price_type_id',
                'cc.discount_percent as cus_discount_percent', 'cc.payment_cat', 'p.name as payment', 'pt.name as price_name')
    ...
    public function edit(Order $order)
    {
        $orderData = DB::table('orders as od')
            ...
            ->select('od.*', 'c.name as cus_name', 'c.phone', 'c.email', 'c.debt as cus_debt',
                'c.total_expenditure', 'c.number_orders', 'c.total_products', 'c.total_return_products', 'c.point',
                'cc.discount_percent as cus_discount_percent', 'adr.address', 'w.name as ward', 'a.name as area', 'cc.name as cus_cat_name', 'cc.price_type_id', 'cc.payment_cat', 'p.name as payment', 'pt.name as price_name')
            ->where('od.id', $order->id)->first();
        ...
        $formattedOrder['products'] = collect($formattedOrder['products'])->map(function ($product) use ($products) {
            $productDetail = collect($products)->firstWhere('SKU', $product['SKU']);
            $product['prices'] = $productDetail ? $productDetail->prices : [];
            return $product;
        })->toArray();

        $customers = Customer::all();
        $price_types = DB::table('price_types as pt')->select('pt.type_id as type_id', 'pt.name as price_name')->get();
        return view('dream-up.pages.order.edit', compact(['formattedOrder', 'products', 'customers', 'price_types']));
    }
    ...
    public function update(Request $request, Order $order)
    {
        ...
        $selectedPrice = $request->selected_price;
        $products = json_decode($request->selected_products, true);
        $existingOrderDetails = OrderDetail::where('order_id', $order->id)
            ->pluck('quantity', 'product_SKU')
            ->toArray(); // Lấy mảng key-value: product_SKU => quantity
        ...
        if (!empty($products) && is_array($products)) {
            foreach ($products as $index => $product) {
                ...
                $newQuantity = (int) $product['count']; // Số lượng mới từ request

                // Dữ liệu cho OrderDetail
                $orderDetailData = [
                    'order_id' => $order->id,
                ...
                if (array_key_exists($productSKU, $existingOrderDetails)) {
                    // Sản phẩm đã tồn tại -> Cập nhật
                    $oldQuantity = $existingOrderDetails[$productSKU]; // Số lượng cũ
                    OrderDetail::where('order_id', $order->id)
                        ->where('product_SKU', $productSKU)
                        ->update($orderDetailData);

                    // Tính toán sự thay đổi số lượng để cập nhật tồn kho
                    $quantityChange = $newQuantity - $oldQuantity;
                } else {
                    // Sản phẩm mới -> Tạo mới
                    $orderDetailData['created_at'] = now();
                    OrderDetail::create($orderDetailData);
                    $quantityChange = $newQuantity; // Số lượng mới được thêm vào
                }

                // Cập nhật số lượng tồn kho
                $quantity = Quantity::where('SKU', $productSKU)->first();
                if ($quantity) {
                    $newStock = $quantity->quantity - $quantityChange;
                    if ($newStock < 0) {
                        throw new \Exception("Số lượng tồn kho của sản phẩm {$productSKU} không đủ.");
                    }
                    $quantity->update(['quantity' => $newStock]);
                } else {
                    throw new \Exception("Không tìm thấy sản phẩm {$productSKU} trong bảng quantities.");
                }
            }
        }

        return redirect()->route('od-index')->with('success', 'Cập nhật đơn hàng thành công');
    }
