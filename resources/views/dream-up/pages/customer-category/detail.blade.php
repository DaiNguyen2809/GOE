...
@section('content')
    <div class="bg-white h-16 w-full flex text-sm">
        ...
        <div class="flex items-center justify-end w-[68%] 2xl:w-[72%]">
            <button type="submit" onclick="handleConfirmDelete('{{ $customerCategory->id }}', 'cm-cat-modal-delete', 'cm-cat-form-delete', '{{ route('cm-cat-destroy', '__id__') }}')" href="{{ route('cm-cat-index') }}" class="w-[12%] h-10 px-4 py-2 ml-4 bg-white border border-red-600 text-red-600 font-gilroy rounded-md cursor-pointer hover:bg-red-600 hover:text-white text-center"><i class="fa-solid fa-ban mr-2"></i>Xóa</button>
            <button type="submit" form="cm-cat-form-update" class="w-[12%] h-10 px-4 py-2 ml-4 bg-white border border-green-600 text-green-600 font-gilroy rounded-md cursor-pointer hover:bg-green-600 hover:text-white text-center"><i class="fa-solid fa-check mr-2"></i>Lưu</button>
        </div>
    </div>

    <form action="{{ route('cm-cat-update', $customerCategory->id) }}" id="cm-cat-form-update" method="POST" class="w-[96%] flex justify-between text-sm">
        @csrf
        @method('PUT')
        <div class="bg-white w-[49%] h-full font-gilroy mt-6 rounded-md shadow-lg">
            ...
            <div class="w-full mt-4">
                <div class="w-full px-6">
                    ...
                    <input class="mt-2 w-full h-10 pl-3 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" type="text" placeholder="Nhập tên nhóm sản phẩm" name="name" value="{{ old('name', $customerCategory->name) }}">
                </div>
                <div class="w-full px-6 mt-4">
                    ...
                    <input class="mt-2 w-full h-10 pl-3 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" type="text" placeholder="Nhập mã nhóm sản phẩm" name="id" value="{{ old('id', $customerCategory->id) }}">
                </div>
                <div class="w-full px-6 mt-4">
        ...
        <div class="bg-white w-[49%] h-full font-gilroy mt-6 rounded-md shadow-lg">
            ...
            <div class="w-full mt-4">
                ...
                <div class="w-full px-6 mt-4">
                    ...
                    <input class="text-right mt-2 w-full h-10 pl-3 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" type="text" placeholder="Nhập mã nhóm sản phẩm" name="discountPercent" value="{{ old('discountPercent', $customerCategory->discount_percent ?? 0) }}">
                </div>
                <div class="w-full px-6 mt-4">
